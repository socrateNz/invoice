...
const P='#1a2e5a'; const G='#d4a017'; const W='#fff'; const B='#111'; const GR='#6b7280'; const BD='#e2e8f0';
...
const s=StyleSheet.create({
  ...
  partyBox:{flex:1,border:`1pt solid ${BD}`,padding:8},
  pHdr:{fontSize:8.5,fontFamily:'Helvetica-Bold',color:P,marginBottom:6,textTransform:'uppercase',borderBottom:`1pt solid ${G}`,paddingBottom:2},
  pLine:{flexDirection:'row',marginBottom:4},pLbl:{width:'35%',fontSize:7.5,color:GR,fontFamily:'Helvetica-Bold'},
  pVal:{width:'65%',fontSize:8,color:B},
  ...
  totV:{backgroundColor:W,color:P,padding:'5 12',fontSize:10,fontFamily:'Helvetica-Bold'},
...
export default function FicheBesoinPDF({data}:{data:FicheBesoinData}){
  ...
  return(<Document><Page size="A4" style={s.page}>
    ...
    <View style={s.partyRow}>
      <View style={s.partyBox}>
        <Text style={s.pHdr}>Demandeur</Text>
        <View style={s.pLine}><Text style={s.pLbl}>Nom & Prénom :</Text><Text style={s.pVal}>{v(data.demandeurNom)}</Text></View>
        <View style={s.pLine}><Text style={s.pLbl}>Fonction :</Text><Text style={s.pVal}>{v(data.demandeurFonction)}</Text></View>
        <View style={s.pLine}><Text style={s.pLbl}>Service :</Text><Text style={s.pVal}>{v(data.departementDemandeur)}</Text></View>
      </View>
      <View style={s.partyBox}>
        <Text style={s.pHdr}>Détails de la demande</Text>
        <View style={s.pLine}><Text style={s.pLbl}>Motif / Projet :</Text><Text style={s.pVal}>{v(data.motifBesoin)}</Text></View>
        <View style={s.pLine}><Text style={s.pLbl}>Date souhaitée :</Text><Text style={s.pVal}>{v(data.dateSouhaitee)}</Text></View>
      </View>
    </View>
    ...
    <View style={{border:`1pt solid ${BD}`,marginBottom:6}}>
      <View style={s.tHead}><Text style={s.thN}>N°</Text><Text style={s.thD}>Désignation de l'article / besoin</Text><Text style={s.thU}>Unité</Text><Text style={s.thQ}>Qté</Text><Text style={s.thP}>Est. Unit.</Text><Text style={s.thT}>Total Est.</Text></View>
