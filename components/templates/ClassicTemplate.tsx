...
const ClassicTemplate = forwardRef<HTMLDivElement, TemplateProps>(({ data }, ref) => {
  ...
  return (
    <div className="bg-[#ffffff] shadow-lg mx-auto w-[210mm] min-h-[297mm] relative" style={{ padding: '0' }}>
      <div ref={ref} className="p-[40px] bg-[#ffffff] h-full w-full text-[#1f2937] text-sm" style={{ fontFamily: data.fontFamily || "Arial, Helvetica, sans-serif" }}>

        {/* Header */}
        <header className="flex justify-between items-start mb-8">
          <div className="bg-[#1e3a8a] text-[#ffffff] p-6 w-[45%] rounded-br-lg">
            <h1 className="text-3xl font-bold uppercase tracking-wider mb-1">{data.senderName || 'NOM ENTREPRISE'}</h1>
            <p className="text-sm mb-4 opacity-90">{data.senderSlogan || 'Slogan de l\'entreprise'}</p>
            <div className="text-xs space-y-1">
              <p>Email : {data.senderEmail || 'email@exemple.com'}</p>
              <p>Tél : {data.senderPhone || '+000 000 000 000'}</p>
            </div>
          </div>
          <div className="text-right w-[50%] pt-4">
            <h2 className="text-3xl font-bold text-[#1e3a8a] uppercase mb-4 tracking-wide">Facture Proforma</h2>
            <div className="text-sm text-[#4b5563] space-y-1">
              <p>N° : <span className="font-semibold text-[#1f2937]">{data.invoiceNumber || '---'}</span></p>
              <p>Date : <span className="font-semibold text-[#1f2937]">{data.invoiceDate || '---'}</span></p>
              <p>Validité : <span className="font-semibold text-[#1f2937]">{data.invoiceValidity || '---'}</span></p>
            </div>
          </div>
        </header>
